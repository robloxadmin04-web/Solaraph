...
--
-- STRICT VM MODE: kapag may hindi suportadong construct, hindi ibinabalik
-- ang original AST. I-aabort ang transform para walang raw source fallback.
--
...
local function fail(reason)
  if reason ~= nil then
    error(UNSUPPORTED .. ":" .. tostring(reason), 0)
  end
  error(UNSUPPORTED, 0)
end
...
function VM.transform(ast)
  ...
  if not ok then
    VM._payload = nil
    VM._last_error = tostring(res)

    -- IMPORTANT:
    -- Never return the original AST here. Returning it would allow the
    -- generator to emit the original source as ordinary Lua.
    error("Solaraph VM transform aborted: " .. VM._last_error, 0)
  end

  local opmapParts = {}
  for logical = 1, 34 do
    table.insert(opmapParts, OPMAP[logical])
  end

  VM._payload = {
    protoData = serializeProtos(),
    mainIndex = res,
    opmap = "{" .. table.concat(opmapParts, ",") .. "}"
  }

  -- This is the only Raw node intentionally generated by this VM pass.
  -- It is the VM entrypoint, not original source.
  ast.body = {
    {
      kind = "CallStatement",
      call = { kind = "Raw", text = "__vmmain()" }
    }
  }
...
-- Returns the last VM transform error without exposing original source text.
function VM.lastError()
  return VM._last_error
end
